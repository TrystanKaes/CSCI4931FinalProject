...
class Optimizer:
    ...
    def getAvgScore(self, population):
        return float(sum([individual["health_score"] for individual in population]) / len(population))

    def evolve(self, population):
        sortedPopulation = sorted(population, key=lambda k: k['health_score'], reverse=True) 
        ...
        print(f"Population size after selecting healthy parents is {len(parents)}")
        ...
        print(f"Population size after a random selection of unhealthy parents is {len(parents)}")
